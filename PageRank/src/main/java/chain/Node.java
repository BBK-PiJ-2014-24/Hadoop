	...
	*/
	 ...
	 */
		...
		public static Node create(String s){
			...
			switch(nt){
				case "CompleteStructure": n.setNodeType(NodeType.CompleteStructure); break;
				case "ProbMass": n.setNodeType(NodeType.ProbMass); break;
				case  "AdjList" : n.setNodeType(NodeType.AdjList); break;
			}
			...
			// pageRank
			float pm = Float.parseFloat(st.nextToken());
			n.setPageRank(pm);
			
			//AdjList
			if(st.hasMoreTokens()){
				String strArr = st.nextToken();
				//System.out.println("strARR " + strArr.replaceAll("\\[", "").replaceAll("\\]", ""));
				String[] items = strArr.replaceAll("\\[", "").replaceAll("\\]", "").split(", ");
				...
				for (int i = 0; i < items.length; i++) {
				    try {
				        ...
				        //System.out.println("ARRAY results " + items[i] );
				    } catch (NumberFormatException nfe) {};
				}
				n.setAdjList(results);
			}
