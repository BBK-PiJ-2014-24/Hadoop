...
public class ChapterRecordReader extends RecordReader<LongWritable, Text> {
   ...
   // private int maxLengthRecord;
    private static final Log LOG = LogFactory.getLog(ChapterRecordReader.class);
	...
	public void initialize(InputSplit genericSplit, TaskAttemptContext context) throws IOException, InterruptedException {
       ...
       delimiterPattern = Pattern.compile("Chapter.*");
	...
	private int readNext(Text text, int maxLineLength, int maxBytesToConsume) throws IOException {
			...
			for (int i = 0; i < maxBytesToConsume; i++) {
				...
				if (m.find()) {
					System.out.println("----- FOUND CHAPTER - YES" );
				    break;
				} else {
